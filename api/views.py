...
class StorageItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if order:
            # Обработка сортировки по полям продукта
            if order == 'product_category_name':
                prefix = '-' if order.startswith('-') else ''
                queryset = queryset.order_by(f'{prefix}product__category__name')
            elif order == 'product_modification':
                prefix = '-' if order.startswith('-') else ''
                queryset = queryset.order_by(f'{prefix}product__description')
            else:
    ...
    def update(self, request, *args, **kwargs):
        ...
        storage_item = StorageItem.objects.filter(pk=storage_item_id).first()

        if not storage_item:
            return Response({"detail": "Не найдено"}, status=status.HTTP_404_NOT_FOUND)

        # Получаем старые данные перед обновлением
        old_count = storage_item.count

        # Обновляем объект
        result = super().update(request, *args, **kwargs)

        # Проверяем, нужно ли записывать лог
        new_count = result.data['count']
        if old_count != new_count:
            Operation.objects.create(
                username=get_username_for_operation(request.user),
                operation=(
                    f'Изменено количество товара на складе {storage_item.product.title} '
...
def import_products(request):
    ...
    if not uploaded_file:
        return Response(data='Файл не был загружен.', status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        for _, row in df.iterrows():
            category_name = row.get('Категория товара')  # Получаем название категории
            category = Category.objects.filter(name=category_name).first()  # Ищем категорию в базе данных

            # Если категория не найдена, используем первую категорию в базе данных
            if not category:
                category = Category.objects.first()  # Можно изменить логику, если требуется специфическая категория

            title = row['Наименование']
            description = row['Модификация']
            price = row['Цена']
...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = CustomPagination # Можно использовать пагинацию, если категорий много
    filter_backends = [SearchFilter]
